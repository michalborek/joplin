...
interface AuthoriseApiResponse {
    code: string;
    hostname: string;
    locationid: number;
    state: string;
...
export default class PCloudApiNodeUtils {

	private api_: PCloudApi;
	private oauthServer_: any = null;
	...
	async oauthDance(targetConsole: any = null) {
		...
		const authCodeUrl = this.api.authCodeUrl(`http://localhost:${port}/return`);

		return new Promise((resolve, reject) => {
			this.oauthServer_ = http.createServer();
			const errorMessage: string = null;

			this.oauthServer_.on('request', (request: any, response: any) => {
				...
				const query = url.query as AuthoriseApiResponse;

				if (!query.code) return writeResponse(400, '"code" query parameter is missing');

				this.api
					.execTokenRequest(query.code, query.hostname)
